...
const QString& k_clientId = "cd630c5b";
}  // namespace
...
QVariant AudioSearchModel::data(const QModelIndex& index, int role) const
{
    if (index.isValid() && index.row() >= 0 && index.row() < m_audioList.size())
    {
        ...
        {
            case AudioNameRole: return audioInfo->title();
            case AudioAuthorRole: return audioInfo->authorName();
...
{
    if (m_isSearching == newIsSearching) return;
